      ...
      <div class="card-body">
        <form class="form-inline" v-on:submit.prevent="onSubmit">
          ...
          <div class="form-group">
            ...
            <textarea v-model="productData.product_description" type="text" class="form-control ml-sm-2 mr-sm-4 my-2" required></textarea>
          </div>
          <div class="form-group">
            <label>Image</label>
            <input v-model="productData.product_image" type="text" class="form-control ml-sm-2 mr-sm-4 my-2" required>
          </div>
            ...
            <tbody>
              <tr v-for="product in sortedProducts" v-bind:key="product.id">
                <template v-if="editId == product.id">
                  ...
                  <td><textarea v-model="editProductData.product_description" type="text"></textarea></td>
                  <td><input v-model="editProductData.product_image" type="text"></td>
                  <td>
                    <span class="icon">
...
export default {
  ...
  methods: {
    ...
    onSubmit(){
      db.firestore().collection('products').add(this.productData).then(this.getProducts)
    ...
    // onDelete(product_id){
    //   db.firestore().collection('products').where('product_id', '==', product_id).get().then(querySnapshot =>{
    //     querySnapshot.forEach(doc=>{
    ...
    // }
    onDelete(id){
      db.firestore().collection('products').doc(id).delete().then((data)=> {
          this.getProducts()
      })
    },
    onEdit(product){
      ...
      this.editProductData.product_description = product.product_description
      this.editProductData.product_image = product.product_image
    },
    onCancel(){
      this.editId = ''
    ...
    onEditSubmit (id){
      db.firestore().collection("products").doc(id).set(this.editProductData).then(
        this.getProducts)
        this.editId = ''
        this.editProductData.product_id = ''
        this.editProductData.product_name = ''
